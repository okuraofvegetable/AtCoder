...
ModInt rui[405][405];
ModInt dp[405][405];
ModInt f(int k,int a,int b){
	if(a==0)return rui[k][b];
	else return rui[k][b]-rui[k][a-1];
}
int main(){
	init();
	cin >> N >> C;
	...
	for(int i=0;i<=400;i++){
		for(int j=0;j<=400;j++){
			if(i==0)rui[i][j]=ModInt(1ll);
			else rui[i][j]=rui[i-1][j]*ModInt(j);
		}
	}
	for(int i=0;i<=400;i++){
		for(int j=0;j<400;j++){
	...
	for(int i=0;i<=C;i++){
		for(int j=1;j<=N;j++){
			for(int k=0;k<=i;k++){
				dp[i][j]+=dp[i-k][j-1]*f(k,A[j-1],B[j-1]);
				//cout << i << j << k << ' ' << dp[i-k][j-1] << endl; 
			}
			//cout << dp[i][j] << endl;
		}
		//cout << endl;
