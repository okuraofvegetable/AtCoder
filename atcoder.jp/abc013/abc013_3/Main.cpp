...
void solve() {
  ...
  for (int i = 0; i <= N; i++) {
    ll rem = N - i;
    if (rem * B + H <= E * (ll)i) continue;
    ll l = -1, r = rem;
    while (r - l > 1ll) {
      ll mid = (l + r) / 2ll;
      if ((rem - mid) * D + mid * B + H > E * (ll)i)
        r = mid;
      else
        l = mid;
    }
    chmin(ans, r * A + (rem - r) * C);
  }
  cout << ans << endl;
  return;
}
