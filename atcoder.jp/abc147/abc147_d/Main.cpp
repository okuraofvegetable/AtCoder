...
ll A[300100];
ll B[300100][60];
ModInt cnt(int bit,int l,int r,int sgn){
  ll diff = 0ll;
  if(l==0)diff = B[r][bit];
  else diff = B[r][bit]-B[l-1][bit];
  if(sgn)return ModInt(diff);
  else return ModInt(r-l+1-diff);
}
...
signed main(){
  ...
  for(int i=0;i<N;i++){
    for(int j=0;j<60;j++){
      ModInt base(1LL<<j);
      if((A[i]>>j)&1ll){
        ModInt c = cnt(j,i+1,N-1,0);
        ans += base * c;
      }else{
        ModInt c = cnt(j,i+1,N-1,1);
        ans += base * c;
      }
    }
  }
