...
template<class T>
vector<T> vect(int len,T elem){ return vector<T>(len,elem); }
...
signed main(){
  ...
  Util::compress(zip);
  auto vec = vect(zip.size(),vect(0,mp(0ll,0ll)));
  for(int i=0;i<=N;i++){
    int k = ((ll)i*X%D+D)%D;
    k = lower_bound(all(zip),k)-zip.begin();
    vec[k].emplace_back(((i-1ll)*i/2ll+(i*X/D)),((N-1ll)*i-(i-1ll)*i/2ll)+(i*X/D));
  }
  // cout << vec << endl;
  ll ans = 0ll;
