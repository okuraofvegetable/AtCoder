...
signed main(){
  ...
  dp[0][0][A[0][0]-B[0][0]+geta]=1;
  dp[0][0][B[0][0]-A[0][0]+geta]=1;
  for(int i=0;i<H;i++){
    for(int j=0;j<W;j++){
      int d = abs(A[i][j]-B[i][j]);
      if(i-1>=0){
        for(int k=-12800;k<=12800;k++){
          dp[i][j][k+d+geta] |= dp[i-1][j][k+geta];
          dp[i][j][k-d+geta] |= dp[i-1][j][k+geta];
        }
      }
      if(j-1>=0){
        for(int k=-12800;k<=12800;k++){
          dp[i][j][k+d+geta] |= dp[i][j-1][k+geta];
          dp[i][j][k-d+geta] |= dp[i][j-1][k+geta];
        }
      }
    }
