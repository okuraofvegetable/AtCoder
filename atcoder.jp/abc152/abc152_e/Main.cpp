...
signed main(){
  ...
  Util::compress(ps); 
  vector<int> div(ps.size());
  for(int i=0;i<N;i++){
    vector<int> cnt(ps.size());
    int d = A[i];
    while(prime[d]>1){
      int idx = lower_bound(all(ps),prime[d])-ps.begin();
      cnt[idx]++;
      d /= prime[d];
    }
    if(d>1){
      int idx = lower_bound(all(ps),d)-ps.begin();
      cnt[idx]++;
    }
    for(int j=0;j<cnt.size();j++){
      div[j] = max(div[j],cnt[j]);
    }
  }
  // cout << ps << endl;
  // cout << div << endl;
  ...
  for(int i=0;i<ps.size();i++){
    L *= pow(mint(ps[i]),div[i]);
  }
  //dmp(L);
  for(int i=0;i<N;i++){
    ans += inv[A[i]];
    // cout << inv[A[i]] << ' ' << inv[A[i]]*L << endl;
  }
  ans *= L;
  cout << ans << endl;
  return 0;
