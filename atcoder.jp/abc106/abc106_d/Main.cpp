...
void solve() {
  ...
  vector<int> ans(Q);
  for (auto t : train) { seg.update(t.second, seg.get_point(t.second) + 1); }
  for (auto q : qs) {
    while (idx < M && train[idx].first < q.l) {
      int end = train[idx].second;
      seg.update(end, seg.get_point(end) - 1);
      idx++;
    }
    ans[q.id] = seg.query(q.l, q.r + 1);
  }
