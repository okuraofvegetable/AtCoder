...
void solve() {
  ...
  T.dfs();
  vector<deque<vector<mint>>> dp(N + 1);
  vector<int> cnt_depth(N + 1);
  function<void(int, int, int)> dfs = [&](int v, int p, int d) {
    ...
    for (auto e : T.G[v]) {
      if (e.to == p) continue;
      dfs(e.to, v, d + 1);
      if (dp[v].size() < dp[e.to].size()) swap(dp[v], dp[e.to]);
      chmax(mx, (int)dp[e.to].size());
      for (int i = 0; i < dp[e.to].size(); i++) {
        auto X = dp[v][i];
        auto Y = dp[e.to][i];
        for (int j = 0; j < 3; j++) dp[v][i][j] = mint(0);
        for (int j = 0; j < 3; j++) {
          for (int k = 0; k < 3; k++) {
            dp[v][i][min(j + k, 2)] += X[j] * Y[k];
          }
        }
      }
    }
    ...
    dp[v].push_front({mint(1), mint(1), mint(0)});
  };
  dfs(0, -1, 0);
  // for (auto &col : ns) cout << col << endl;
  mint ans(0);
  // for (int i = 0; i < dp[0].size(); i++) cout << dp[0][i] << ' ';
  // cout << endl;
  for (int i = 0; i < dp[0].size(); i++) {
    ans += mint(2).pow(N + 1 - cnt_depth[i]) * dp[0][i][1];
  }
  cout << ans << endl;
  return;
}
