...
void solve() {
  ...
  int N, K;
  cin >> N >> K;
  auto A = vect(K + 1, vect(N, mint(1)));
  for (int i = 0; i < N; i++) {
    int x;
    cin >> x;
    A[1][i] = mint(x);
    for (int j = 2; j <= K; j++) { A[j][i] = A[j - 1][i] * A[1][i]; }
  }
  for (int j = 0; j <= K; j++) { dmp(A[j]); }

  auto B = vect(K + 1, mint(0));
  for (int i = 0; i <= K; i++) B[i] = accumulate(all(A[i]), mint(0));

  for (int i = 1; i <= K; i++) {
    mint ans(0);
    for (int j = 0; j <= i / 2; j++) {
      if (j * 2 != i)
        ans += nCr(i, j) * (B[j] * B[i - j] - B[i]);
      else
        ans += nCr(i, j) * (B[j] * B[i - j] - B[i]) / mint(2);
    }
    cout << ans << endl;
  }
