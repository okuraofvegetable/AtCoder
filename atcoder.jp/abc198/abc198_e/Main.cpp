...
void solve() {
  ...
  function<void(int, int)> dfs = [&](int v, int p) {
    for (auto e : g.G[v]) {
      if (e.to == p) continue;
      dfs(e.to, v);
      if (mss[v].size() < mss[e.to].size()) swap(mss[v], mss[e.to]);
      for (auto ele : mss[e.to]) mss[v].insert(ele);
    }
    auto it = mss[v].lower_bound(pii(C[v], -1));
    while (it->first == C[v]) { it = mss[v].erase(it); }
    mss[v].insert(pii(C[v], v));
  };
  dfs(0, -1);
  vector<int> ans;
  for (auto [col, id] : mss[0]) { ans.push_back(id); }
  sort(all(ans));
