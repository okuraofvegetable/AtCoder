...
void solve() {
  ...
  function<mint(int, int)> dfs = [&](int v, int p) {
    ...
    for (const auto &e : T.G[v]) {
      if (e.to == p) continue;
      dp[v] *= dfs(e.to, v);
      dp[v] *= facinv[T.subtree_size[e.to]];
      sz += T.subtree_size[e.to];
    }
    dp[v] *= fac[sz];
    return dp[v];
  };
  ...
  dmp(dp);
  vector<mint> ep(N);
  function<void(int, int, mint)> efs = [&](int v, int p, mint dpar) {
    ep[v] = dpar * fac[N - 1] * facinv[N - T.subtree_size[v]];
    for (const auto &e : T.G[v]) {
      if (e.to == p) continue;
      ep[v] *= dp[e.to];
      ep[v] *= facinv[T.subtree_size[e.to]];
    }
    for (const auto &e : T.G[v]) {
      if (e.to == p) continue;
      mint data = ep[v] * facinv[N - 1];
      data /= dp[e.to];
      data *= fac[T.subtree_size[e.to]];
      data *= fac[N - 1 - T.subtree_size[e.to]];
      efs(e.to, v, data);
    }
  };
  efs(0, -1, mint(1));
  // dmp(ep);
  for (int i = 0; i < N; i++) { cout << ep[i] << endl; }
  return;
}
