...
signed main(){
  ...
  for(int i=0;i<z.size();i++){
    // Max
    for(int j=1;j<=min(K,z[i].second);j++){
      if(K-j<=sum){
        //cout << z[i].second << ' ' << j << endl;
        ans += mint(z[i].first) * nCr(z[i].second,j)*nCr(sum,K-j);
      }
    }
    sum += z[i].second;
  }
  ...
  for(int i=sz-1;i>=0;i--){
    for(int j=1;j<=min(K,z[i].second);j++){
      if(K-j<=sum)ans -= mint(z[i].first) * nCr(z[i].second,j) * nCr(sum,K-j);
    }
    sum += z[i].second;
  }
  cout << ans << endl;
