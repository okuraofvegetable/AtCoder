...
void solve() {
  ...
  mint a(0), b(0);
  for (int i = 0; i < B + W; i++) {
    if (i == B) a = nCr(i, B) * pow(inv[2], i);
    if (i == W) b = nCr(i, W) * pow(inv[2], i);
    if (i > B) a = (a * pow(mint(2), i) + nCr(i - 1, B - 1)) * pow(inv[2], i);
    if (i > W) b = (b * pow(mint(2), i) + nCr(i - 1, W - 1)) * pow(inv[2], i);
    // dmp(a);
    // dmp(b);
    cout << (mint(1) - a + b) * inv[2] << endl;
  }
  return;
}
