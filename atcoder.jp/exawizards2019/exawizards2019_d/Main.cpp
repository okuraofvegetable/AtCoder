...
void solve() {
  ...
  auto dp = vect(X + 1, vect(N + 1, mint(0)));
  dp[X][0] = mint(1);
  ...
  for (int i = X; i >= 0; i--) {
    if (!xs[i]) continue;
    int K = upper_bound(all(S), i) - S.begin();
    for (int j = 0; j < N; j++) {
      if (N - K >= j) {
        dp[i][j + 1] += dp[i][j] * mint(N - K - j);
        for (int k = 0; k < K; k++) dp[i % S[k]][j + 1] += dp[i][j];
      }
    }
    ans += mint(i) * dp[i][N];
  }
