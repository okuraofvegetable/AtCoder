...
void solve(){
  ...
  sort(all(A));
  for(int i=N;i>=1;i--){
    A[i].second = (A[i].second+A[i-1].second)%2;
  }
  Graph<int> G(N+1);
  for(int i=0;i<M;i++){
    int l,r;
    cin >> l >> r;
    r++;
    l = lower_bound(all(A),P(l,-1))-A.begin();
    r = lower_bound(all(A),P(r,-1))-A.begin();
    if(l!=r){
      G.add_edge_undirected(l,r,1,i+1);
      // dmp(P(l,r));
    }
  }
  // dmp(A);
