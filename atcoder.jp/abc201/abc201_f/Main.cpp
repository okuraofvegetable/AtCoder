...
void solve() {
  ...
  RangeSum<ll> Asum(A), Bsum(B), Csum(C);

  SegmentTree<ll> seg(
      N, [](ll x, ll y) { return max(x, y); }, -LLINF);

  for (int i = 0; i < N; i++) {
    ll val = Asum.sum(0, P[i] - 1) - Bsum.sum(0, P[i] - 1) + A[P[i]];
    ll prev = seg.query(0, P[i]);
    if (prev > 0) chmax(val, prev + A[P[i]]);
    if (seg.get_point(P[i]) < val) seg.update(P[i], val);
  }

  ll ans = Asum.sum(0, N - 1);
  for (int i = 0; i < N; i++) {
    dmp(i, Asum.sum(0, i), seg.get_point(i), Csum.sum(i + 1, N - 1));
    chmin(ans, Asum.sum(0, i) - seg.get_point(i) + Csum.sum(i + 1, N - 1));
  }
