...
void solve() {
  ...
  SegmentTree<int> seg(cntw, [](int x, int y) { return max(x, y); }, -1);
  int ans = 0;
  for (int h_ : h) {
    for (int w_ : g[h_]) { seg.update(w_, seg.get_point(w_) - 1); }
    chmax(ans, cnth[h_] + seg.query(0, MX));
    for (int w_ : g[h_]) { seg.update(w_, seg.get_point(w_) + 1); }
  }
  cout << ans << endl;
  return;
