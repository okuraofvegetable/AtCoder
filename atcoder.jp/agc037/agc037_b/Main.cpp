...
void solve() {
  ...
  //   chmax(c[m[s[i]]], i);
  // }
  // vector<int> rui(s.size(), 0);
  // vector<bool> isB(s.size(), true);
  // for (int i = 1; i <= n; i++) {
  //   rui[a[i]]++;
  //   rui[c[i]]--;
  //   isB[a[i]] = isB[c[i]] = false;
  // }
  // dmp(a, c);
  // dmp(rui);
  // dmp(isB);
  ...
  for (int i = 0; i < s.size(); i++) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (cnt[i] == 0) continue;
      if (best == -1 ||
          __builtin_popcount(best | mask) < __builtin_popcount(i | mask))
        best = i;
    }
    assert(best != -1);
    ans *= cnt[best];
