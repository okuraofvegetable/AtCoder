...
void solve() {
  ...
  for (int i = 0; i < M; i++) {
    ...
    a--;
    b--;
    es[c - 'a'].emplace_back(a, b, 1);
    es[c - 'a'].emplace_back(b, a, 1);
  }
  auto encode = [&](int a, int b) { return a * N + b; };
  ...
  for (int i = 0; i < 30; i++) {
    for (int j = 0; j < es[i].size(); j++) {
      for (int k = 0; k < es[i].size(); k++) {
        edge e = es[i][j];
        edge f = es[i][k];
        int From = encode(e.from, f.from);
        int To = encode(e.to, f.to);
        G[From].push_back(To);
      }
    }
  }
  ...
  int ans = INF;
  for (int i = 0; i < 30; i++) {
    for (auto e : es[i]) { chmin(ans, dist[encode(e.from, e.to)] * 2 + 1); }
  }

  for (int i = 0; i < N; i++) { chmin(ans, dist[encode(i, i)] * 2); }
