...
void solve() {
  ...
  cin >> s;
  int N = s.size();
  auto dp = vect(2 * N + 1, vect(2 * N + 1, mint(0)));
  vector<int> rui(2 * N + 1, 0);
  for (int i = 0; i < 2 * N; i++) {
    if (i < N)
      rui[i + 1] = rui[i] + (s[i] - '0');
    else
  ...
  for (int i = 0; i < 2 * N; i++) {
    for (int j = 0; j < 2 * N; j++) {
      if (j > rui[i + 1]) break;
      int r = min(2 * N, 2 * (i + 1)) - rui[i + 1] - (i - j);
      int b = rui[i + 1] - j;
      if (r > 0) dp[i + 1][j] += dp[i][j];
      if (b > 0) dp[i + 1][j + 1] += dp[i][j];
    }
  }
  // for (const auto &row : dp) dmp(row);
  mint ans(0);
  for (int i = 0; i <= 2 * N; i++) ans += dp[2 * N][i];
  cout << ans << endl;
  return;
}
