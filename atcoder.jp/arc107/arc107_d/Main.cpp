...
void solve() {
  int N, K;
  cin >> N >> K;
  auto dp = vect(N - K + 1, vect(N - K + 1, mint(0)));
  auto rs = vect(N - K + 1, vect(N - K + 1, mint(0)));
  for (int i = 0; i <= N - K; i++) {
    for (int j = 0; j <= i; j++) {
      if (i == j) {
        if (i <= K) dp[i][j] = mint(1);
      } else {
        dp[i][j] = rs[i - j][(j + 1) / 2];
      }
    }
    rs[i][N - K] = dp[i][N - K];
    for (int j = N - K - 1; j >= 0; j--) { rs[i][j] = rs[i][j + 1] + dp[i][j]; }
    // dmp(dp[i]);
  }
  mint ans(0);
  for (int i = 0; i <= N - K; i++) ans += dp[N - K][i];
  cout << ans << endl;
  return;
}
