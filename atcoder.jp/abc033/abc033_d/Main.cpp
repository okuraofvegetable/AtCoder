...
struct Point {
  ...
  int det(const Point &a) const { return x * a.y - y * a.x; }
...
void solve() {
  ...
  ll eikaku = 0;
  for (int i = 0; i < N; i++) {
    vector<pair<double, Point>> qs;
    for (int j = 0; j < N; j++) {
      if (i == j) continue;
      qs.emplace_back(atan2(ps[j].y - ps[i].y, ps[j].x - ps[i].x),
                      ps[j] - ps[i]);
    }
    sort(all(qs));
    int idx1 = 0;
    ...
    for (int j = 0; j < qs.size(); j++) {
      // dmp(qs[j].second.dot(qs[idx1 % qs.size()].second));
      // dmp(qs[j].second.det(qs[idx1 % qs.size()].second));
      while ((idx1 % qs.size() != j || idx1 == j) &&
             qs[j].second.dot(qs[idx1 % qs.size()].second) > 0 &&
             qs[j].second.det(qs[idx1 % qs.size()].second) >= 0)
        idx1++;
      while ((idx2 % qs.size() != j || idx2 == j) &&
             qs[j].second.dot(qs[idx2 % qs.size()].second) >= 0 &&
             qs[j].second.det(qs[idx2 % qs.size()].second) >= 0)
        idx2++;
      assert(idx1 > j);
      assert(idx2 >= idx1);
      eikaku += idx1 - j - 1;
  ...
  ll All = N * (N - 1) * (N - 2) / 2;
  ll donkaku = All - eikaku - chokkaku;
  cout << (eikaku - 2 * (chokkaku + donkaku)) / 3 << ' ' << chokkaku << ' '
       << donkaku << endl;
}
