...
void solve() {
  ...
  auto St = vect(S.size() + 1, vect(17, mint(1)));
  ...
  for (int i = 1; i <= S.size(); i++) {
    for (int j = 1; j <= min(i, 16); j++) {
      if (j == 1 || i == j)
        ...
        St[i][j] = St[i - 1][j - 1] + mint(j) * St[i - 1][j];
      }
    }
    // dmp(St[i]);
  }
  ...
  for (int i = K; i < S.size(); i++) {
    ans += mint(15) * St[i][K] * nPr(15, K - 1);
  }
  dmp(ans);

  int mask = 0;
  for (int i = 0; i < S.size(); i++) {
    if (popcount(mask) > K) break;
    int d = htoi(S[i]);
    for (int j = 0; j < d; j++) {
      if (i == 0 && j == 0) continue;
      int nmask = mask | (1 << j);
      int pnmask = popcount(nmask);
      if (pnmask > K) continue;
      mint ret(0);
      for (int k = 0; k <= K - pnmask; k++) {
        ret += nCr(K - pnmask, k) * mint(K - k).pow(S.size() - i - 1) *
               mint(-1).pow(k);
      }
      ret *= nCr(16 - pnmask, K - pnmask);
      ans += ret;
    }
    mask |= (1 << d);
  }

  if (popcount(mask) == K) ans += mint(1);
