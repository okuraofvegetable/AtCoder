...
signed main(){
  ...
  vector<bool> fnuke(N,false);
  snuke[A] = true;
  fnuke[B] = true;
  for(int i=A;i<N;i++){
    if(snuke[i]){
      if(S[i+1]=='.')snuke[i+1] = true;
      if(S[i+2]=='.')snuke[i+2] = true;
    }
  }
  for(int i=B;i<N;i++){
    if(fnuke[i]){
      if(S[i+1]=='.')fnuke[i+1] = true;
      if(S[i+2]=='.')fnuke[i+2] = true;
    }
  }
  if(C<D){
    if(snuke[C]&&fnuke[D])cout << "Yes" << endl;
    else cout << "No" << endl;
  }else{
    if(!snuke[C]||!fnuke[D]){
      cout << "No" << endl;
    ...
    for(int i=B;i<=D;i++){
      if(fnuke[i]&&snuke[i-1]&&snuke[i+1]){
        cout << "Yes" << endl;
        return 0;
      }
    }
