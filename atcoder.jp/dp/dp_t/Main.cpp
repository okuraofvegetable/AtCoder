...
ModInt dp[3010][3010];
bool fin[3010];
ModInt rec(int pos,int l){
	if(pos==s.size()){
		return dp[pos][l]=ModInt(1ll);
	}
	if(dp[pos][l]!=ModInt(-1ll))return dp[pos][l];
	if(!fin[pos+1]){
		for(int i=0;i<N-pos-1;i++)rec(pos+1,i);
		fin[pos+1]=true;
		for(int i=1;i<N-pos-1;i++)dp[pos+1][i]+=dp[pos+1][i-1];
	}
	ModInt ret;
	if(s[pos]=='<'){
		if(l==0)ret = dp[pos+1][N-pos-2];
		else ret = dp[pos+1][N-pos-2]-dp[pos+1][l-1];
	}else{
		if(l>0)ret = dp[pos+1][l-1];
	}
	return dp[pos][l]=ret;
}
int main(){
