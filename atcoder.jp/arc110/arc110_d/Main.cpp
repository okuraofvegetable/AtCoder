...
void solve() {
  ...
  vector<int> A(N);
  cin >> A;
  // auto dp = vect(N + 1, vect(M + 1, 0ll));
  // dp[0][0] = 1ll;
  // dmp(dp[0]);
  // for (int i = 1; i <= N; i++) {
  //   for (int j = 0; j <= M; j++) {
  //     for (int k = A[i - 1]; k <= j; k++)
  //       dp[i][j] += dp[i - 1][j - k] * nCk(k, A[i - 1]);
  //   }
  //   dmp(dp[i]);
  // }
  // cout << endl;
  // for (int i = 0; i < M; i++) { dp[N][i + 1] += dp[N][i]; }
  // dmp(dp[N]);
