...
signed main(){
  ...
  vector<ModInt> y(N);
  for(int i=0;i<=N-3;i++){
    y[i] = nCr(N-1,i+2) * fac[i] * fac[N-i-3];
  }
  vector<ModInt> xm(N);
  for(int i=0;i<N;i++){
    cin >> x[i];
  ...
  DS::RangeSum<ModInt> rs(xm);
  vector<ModInt> z(N);
  for(int i=0;i<N-2;i++){
    z[i] = rs.sum(1+i,N-2)-rs.sum(0,N-3-i);
  }
  ModInt ans;
  for(int i=0;i<N-2;i++){
    ans += y[i] * z[i];
  ...
  for(int i=0;i+2<N;i++){
    ...
    //   int a = j-i-1;
    //   // for(int k=0;k<=N-3-a;k++){
    //   //   ans += nCr(N-3-a,k)* fac[k+a] * fac[N-3-a-k] * ModInt(x[j]-x[i]);
    //   // }
    //   ans += y[a] * ModInt(x[j]-x[i]);
    // }
    // cout << ans << endl;
    ans += nCr(N-1,i) * fac[N-2-i] * fac[i] * ModInt(x[N-1]-x[i]);
    // cout << ans << endl;
  }
  ans += fac[N-1]*ModInt(x[N-1]-x[N-2]);
  cout << ans << endl; 
  return 0;
}
