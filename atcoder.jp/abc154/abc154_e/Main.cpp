...
string s;
int dp[2][104][4];
int rec(int tight,int digit,int nonzero){
  if(digit==s.size()){
    if(nonzero!=0)return 0;
    else return 1;
  }
  if(nonzero==0)return 1;
  if(dp[tight][digit][nonzero]!=-1)return dp[tight][digit][nonzero];
  int res = 0;
  if(tight){
    // 0
    res += rec((s[digit]=='0')?1:0,digit+1,nonzero);
    // 1ijou
    for(int i=1;i<=(int)(s[digit]-'0');i++){
      res += rec(((int)(s[digit]-'0'))==i?1:0,digit+1,nonzero-1);
    }
  }else{
    // 0
    res += rec(0,digit+1,nonzero);
    // 1ijou
    for(int i=1;i<=9;i++){
      res += rec(0,digit+1,nonzero-1);
    }
  }
  return dp[tight][digit][nonzero]=res;
}
signed main(){
  fastio();
  int k;
