...
signed main(){
  ...
  for(int i=N-2;i>=0;i--){
    ...
    dp[i] /= (double)G[i].size();
    dp[i] += 1.0;
  }
  for(int i=0;i+1<N;i++){
    pair<double,int> mx = make_pair(dp[G[i][0]],0);
    for(int j=0;j<G[i].size();j++){
      chmax(mx,make_pair(dp[G[i][j]],j));
    }
    cand[i] = G[i][mx.second];
  }
  // cout << cand << endl;
  double ans = dp[0];
  for(int i=0;i+1<N;i++){
    ...
    for(int j=N-2;j>=0;j--){
      if(j==i){
        for(auto k : G[j]){
          if(k==cand[j])continue;
          else ep[j] += ep[k];
        }
        ep[j] /= (double)(G[j].size()-1);
        ep[j] += 1.0;
      }else{
        for(auto k : G[j]){
          ep[j] += ep[k];
        }
        ep[j] /= (double)G[j].size();
        ep[j] += 1.0;
      }
    }
    chmin(ans,ep[0]);
