...
void solve() {
  ...
  auto dist = vect(K, vect(K, INF));
  for (int i = 0; i < K; i++) {
    auto d = dijkstra(g, C[i]);
    for (int j = 0; j < K; j++) {
      if (d[C[j]] == 0 || d[C[j]] > INF) continue;
      chmin(dist[i][j], d[C[j]] - 1);
    }
  }
  auto dp = vect((1 << K), vect(K, LLINF));
  ...
  for (int j = 0; j < (1 << K); j++) {
    for (int i = 0; i < K; i++) {
      for (int k = 0; k < K; k++) {
        ...
        chmin(dp[j | (1 << k)][k], dp[j][i] + dist[i][k]);
      }
    }
  }
  ll ans = LLINF;
  for (int i = 0; i < K; i++) { chmin(ans, dp[(1 << K) - 1][i]); }
  if (ans == LLINF)
    cout << -1 << endl;
  else
    cout << ans + K << endl;
