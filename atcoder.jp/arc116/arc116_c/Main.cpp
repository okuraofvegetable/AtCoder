...
void solve() {
  int N, M;
  cin >> N >> M;
  auto dp = vect(M + 1, vect(19, mint(0)));
  for (int i = 1; i <= M; i++) dp[i][1] = mint(1);

  for (int j = 1; j + 1 < 19; j++) {
    for (int i = 1; i <= M; i++) {
      for (int k = i * 2; k <= M; k += i) { dp[k][j + 1] += dp[i][j]; }
    }
  }

  for (int j = 1; j < 19; j++) {
    for (int i = 1; i <= M; i++) { dp[0][j] += dp[i][j]; }
  }

  mint ans(0);

  for (int i = 1; i < 19; i++) {
    if (i <= N) ans += nCr(N - 1, i - 1) * dp[0][i];
  }
